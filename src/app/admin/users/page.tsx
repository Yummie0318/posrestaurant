...
export default async function UsersPage({ searchParams }: { searchParams: Promise<UserSearchParams> }) {
  ...
  return (
    <AppShell pathname="/admin/users" user={user} title="Users" subtitle="Control roles and account access with a shared admin UI system.">
        ...
        <Card className="shrink-0">
          <CardContent className="p-4">
            <form className="flex gap-2">
              <Input name="q" defaultValue={params.q ?? ''} placeholder="Search name, username, or role" className="flex-1" />
        ...
        <Card className="flex min-h-0 flex-col overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-3">
            <div>
              ...
              <CardDescription>{filteredUsers.length} account{filteredUsers.length === 1 ? '' : 's'}</CardDescription>
            </div>
            <Link href={buildHref({ q: params.q, userModal: 'create' })}>
              <Button size="icon" aria-label="Add user" title="Add User">
          ...
          <CardContent className="min-h-0 flex-1 overflow-auto">
            {filteredUsers.length ? (
                  ...
                  {filteredUsers.map((account) => (
                    <div key={account.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge variant={account.active ? 'success' : 'outline'}>{account.active ? 'Active' : 'Inactive'}</Badge>
                      </div>
                      <div className="mt-3 flex items-center gap-2">
                        <Badge variant="outline">{account.role}</Badge>
                      </div>
                      <div className="mt-4 flex gap-2">
                        <Link href={buildHref({ q: params.q, userModal: 'edit', userId: account.id })}>
                          <Button type="button" variant="outline" size="icon" aria-label="Edit user"><Pencil className="h-4 w-4" /></Button>
                        </Link>
                        <form action={deleteUserAction}>
                          <input type="hidden" name="id" value={account.id} />
                          <PendingIconButton variant="destructive" size="icon" aria-label="Delete user" pendingLabel="Deleting user">
                    ...
                    <tbody>
                      {filteredUsers.map((account) => (
                        <tr key={account.id} className="border-t border-slate-100">
                          ...
                          <TD><Badge variant="outline">{account.role}</Badge></TD>
                          <TD><Badge variant={account.active ? 'success' : 'outline'}>{account.active ? 'Active' : 'Inactive'}</Badge></TD>
                          <TD>
              ...
              <EmptyState title="No users found" description="Try a broader search or create a new staff account." />
            )}
          </CardContent>
        </Card>
      </div>
